  ...
  const self: World = {
    ...
    fetch: (async (input, init) => {
      ...
      if (url.startsWith(IDENTITY)) {
        ...
        self.exchanges += 1
        const token = await new SignJWT({ typ: 'service', scopes: ['ledger:read', 'ledger:post', 'ledger:reserve', 'custody:sign:deployer', 'indexer:read'], jti: `t-${++jti}` })
          .setProtectedHeader({ alg: 'RS256', kid: 'k1' })
          .setIssuedAt()
          .setIssuer(ISSUER)
          .setAudience(AUDIENCE)
          .setSubject('service:mint')
          .setExpirationTime(Math.floor(Date.now() / 1000) + SERVICE_TTL_SECONDS)
          .sign(privateKey)
        return new Response(
          JSON.stringify({ token, service: 'mint', scopes: ['ledger:read', 'ledger:post', 'ledger:reserve', 'custody:sign:deployer', 'indexer:read'], expiresIn: SERVICE_TTL_SECONDS }),
...
test('this service still authenticates ELEVEN MINUTES after boot — the ten-minute cliff', async (t) => {
  ...
  await upstreams.indexer.token('eth', 'testnet', '0xabc')
...
test('an unreachable identity is a 503 to the caller, never an unauthenticated peer call', async (t) => {
  ...
  await assert.rejects(() => upstreams.indexer.token('eth', 'testnet', '0xabc'))
